/*
	SourceGear Vault
	Copyright 2002-2005 SourceGear Corporation
	All Rights Reserved.
...
namespace VaultCmdLineClientNs
{
	...
	public class VaultCmdLineClient
	{
				...
				switch (curArg.Cmd)
				{
					...
					case Command.GET:
					{
						...
						if ( _args.DestPath != null && _args.WorkingFolder != null )
							throw new UsageException("The -destpath and -workingfolder options are mutually exclusive: only one may be used.");
					...
					case Command.SETWORKINGFOLDER:
					{
						...
						SetWorkingFolder((string) curArg.items[0], (string) curArg.items[1], true);
		...
		bool ProcessCommandGet(ArrayList strItemArray)
		{
			...
			// check for flag to use/not use cloak 
			...
			string strOnlyReposItem = (string)strItemArray[0];
			string originalWorkingFolder = null;
			VaultClientTreeObject workingFolderTreeObject = null;
			bool bResetWorkingFolder = false;
			if ( _args.WorkingFolder != null )
			{
				// make sure the item exists
				workingFolderTreeObject = _ci.TreeCache.Repository.Root.FindTreeObjectRecursive(strOnlyReposItem);
				if ( workingFolderTreeObject == null )
					throw new Exception(strOnlyReposItem + " does not exist in repository " + _args.Repository);
				...
				originalWorkingFolder = _ci.TreeCache.GetWorkingFolder(workingFolderTreeObject.FullPath);

				if ( originalWorkingFolder == null || NormalizeFolderPath(originalWorkingFolder) != NormalizeFolderPath(_args.WorkingFolder) )
				{
					bResetWorkingFolder = true;
					SetWorkingFolder(workingFolderTreeObject.FullPath, _args.WorkingFolder, true);
				}
			}
			...
			try
			{
				bSuccess = performGet(strItemArray);
			}
			finally
			{
				// reset prev cloak value
				if ( (_args.RespectCloaks == false) && (bResetCloaks == true) )
					_ci.TreeCache.Cloaks = cloaks;
				// reset prev MakeBackup value
				if (_args.MakeBackup != BackupOption.usedefault)
					_ci.WorkingFolderOptions.MakeBackups = saveMakeBackupOption;
				// Reset the previous working folder.
				if ( bResetWorkingFolder )
				{
					if ( originalWorkingFolder == null )
						_ci.TreeCache.RemoveWorkingFolder(workingFolderTreeObject.FullPath);
					else
						_ci.TreeCache.SetWorkingFolder(workingFolderTreeObject.FullPath, originalWorkingFolder);
				}
			}
		...
		bool performGet(ArrayList strItemArray)
		{
			...
			if (wf == null)
				if (isCheckout)
					_xml.WriteUserMessage("There is no working folder specified for " + obj.FullPath + "\nYou can set one using " 
						+ Command.SETWORKINGFOLDER + " or use a temporary working folder by using the -workingfolder option with GET.");
				else
					_xml.WriteUserMessage("There is no working folder specified for " + obj.FullPath + "\nYou can set one using " 
						+ Command.SETWORKINGFOLDER + ", use a temporary working folder by using the -workingfolder option with GET, or use the " 
						+ Option.DESTPATH + " option to get \nto a non-working folder");
			return (wf != null);
		}
		...
		public bool ProcessCommandGetVersion(int version, string strReposItem, string strDestFolder)
		{
			...
			bool useWorkingFolder = _args.useWorkingFolder;
			if( Misc.stringIsBlank(strDestFolder) )
			...
			VaultClientTreeObject treeObjectToRetrieve = _ci.TreeCache.Repository.Root.FindTreeObjectRecursive(strReposItem);
			if ( treeObjectToRetrieve == null )
				throw new Exception(string.Format("{0} does not exist", strReposItem));
			...
			string originalWorkingFolder = null;
			VaultClientTreeObject workingFolderTreeObject = treeObjectToRetrieve;
			bool bResetWorkingFolder = false;
			if ( useWorkingFolder && !Misc.stringIsBlank(strDestFolder) )
			{
				if ( treeObjectToRetrieve is VaultClientFile )
					workingFolderTreeObject = treeObjectToRetrieve.Parent;

				// You have to use TreeCache.GetWorkingFolder here because it retrieves only directly
				// assigned working paths, not inherited working paths, and that's what we want.
				originalWorkingFolder = _ci.TreeCache.GetWorkingFolder(workingFolderTreeObject.FullPath);

				if ( Misc.stringIsBlank(originalWorkingFolder) || NormalizeFolderPath(originalWorkingFolder) != NormalizeFolderPath(strDestFolder) )
				{
					bResetWorkingFolder = true;
					SetWorkingFolder(treeObjectToRetrieve.FullPath, strDestFolder, true);
				}
			}
			...
			try
			{
				bSuccess = performGetVersion(treeObjectToRetrieve, version, useWorkingFolder, mt, strDestFolder);
			}
			finally
			{
				if (!_args.RespectCloaks && bResetCloaks) 
					_ci.TreeCache.Cloaks = cloaks;
		...
		private bool performGetVersion(VaultClientTreeObject treeObjectToRetrieve, int version, bool useWorkingFolder, MergeType mt, string strDestFolder)
		{
			...
			if (treeObjectToRetrieve is VaultClientFolder)
			{
				...
				try
				{
					_ci.Connection.GetBranchStructure(_ci.ActiveRepositoryID, treeObjectToRetrieve.FullPath, vcFolder.ID, version, ref vfDelta, false);
				}
				catch
				{
					if ( vfDelta.ObjVerID < 1 )
						throw new Exception("There is no version " + version + " of " + treeObjectToRetrieve.FullPath + " in " + _ci.Repository.RepName + ".");
					else
						throw;
				}
				...
				if ( useWorkingFolder )
				{
					...
					getResponses = _ci.Get(vcFolder, _args.Recursive, false, _args.MakeWritable, _args.SetFileTime, mt, null);
				}
				else
					getResponses = _ci.GetByDisplayVersionToNonWorkingFolder(vcFolder, _args.Recursive, _args.MakeWritable, _args.SetFileTime, strDestFolder, null);
			}
			else
			{
...
#region Exclude Actions
		...
		// If the provided path ends with a slash or backslash, remove it
